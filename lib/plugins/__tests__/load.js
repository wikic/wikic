...
jest.mock('../../utils/getClosestConfigFromCache', () =>
  jest.fn((srcDir, site) => Object.assign({}, site)));
...
describe('load Plugins', () => {
  ...
  it('invokes getClosestConfig default', async () => {
    ...
    expect(getClosestConfig).toHaveBeenCalledWith('/path/to', site);
  });
  ...
  it('invokes getClosestConfigFromCache if has configCaches', async () => {
    ...
    expect(getClosestConfigFromCache).toHaveBeenCalledWith('/path/to', site, configCaches);
  });
});

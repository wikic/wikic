...
jest.mock('fs-promise', () => {
  const fsp = {}
  fsp.readFile = () => Promise.resolve('string')
  fsp.readJson = () => Promise.resolve({ field: 'a' })
  return fsp
})
...
describe('loadConfig', () => {
  it('return a doge', async () => {
    expect(await loadConfig('/path/to/', {})).toEqual({})
    expect(await loadConfig('/path/to/', {})).toEqual({ field: 'a' })
  })

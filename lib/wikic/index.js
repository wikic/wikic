...
class Wikic {
  ...
  watch() {
    chokidar
      ...
      .on('change', (filePath) => {
        ...
        Object.assign(matchers, this.config.watchHandlers)
  ...
  async buildStaticFiles() {
    const { docsPath } = this.config
    const commonIgnored = getStaticIgnored(this.config)
    const rootStaticIgnored = Array.from(commonIgnored)
    rootStaticIgnored.push(`${docsPath}/**`)
    rootStaticIgnored.push('_*/**')
    commonIgnored.push(fixUnderscores(docsPath))

    const files = await glob('**/*', {
      cwd: this.root,
      nodir: true,
      ignore: rootStaticIgnored,
    })

    const docsStaticFiles = await glob(`${docsPath}/**/!(*.md)`, {
      cwd: this.root,
      nodir: true,
      ignore: commonIgnored,
    })

    await Promise.all([
      ...files.map(this.buildStaticFile),
      ...docsStaticFiles.map(async (filePath) => {
        const src = path.join(this.root, filePath)
        const dist = path.join(this.publicPath, filePath.replace(`${docsPath}/`, ''))

  ...
  watch() {
    chokidar
      ...
      .on('change', (filePath) => {
        ...
        Object.assign(handlers, this.config.watchHandlers)

        if (anymatch(handlers.setupAndBuild, filePath)) {
          logger.verbose(`[watch]: setupAndBuild because ${filePath} changed`)
          this.setup().build()
        } else if (anymatch(handlers.build, filePath)) {
          logger.verbose(`[watch]: build because ${filePath} changed`)
          this.build()
        } else if (
          !anymatch(getStaticIgnored(this.config), filePath) &&
          anymatch(handlers.buildStaticFile, filePath)
        ) {
          logger.verbose(`[watch]: buildStaticFile because ${filePath} changed`)

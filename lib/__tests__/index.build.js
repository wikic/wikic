...
describe('wikic build', () => {
  ...
  describe('nunjucks', () => {
    ...
    it('relative', () => {
      const wikic = new Wikic();
      wikic.publicPath = path.resolve('a', 'b');
      wikic.config.baseurl = 'base';
      wikic.renderer.addFilter.mockClear();
      wikic.renderMD({ data: '', page: {}, renderContext: {}, dist: path.resolve('a', 'b', 'c', 'd.html') });
...
describe('watch', () => {
  it('works', () => {
    ...
    expect(fsp.removeSync.mock.calls[0][0]).toMatch(path.join('docs', 'text.txt'));

...
class Wikic {
  ...
  renderMD(context) {
    const { data: oData, page, renderContext: oRenderContext, dist } = context;
    const { layout } = page;
    const renderContext = Object.assign(oRenderContext, {
      content: oData,
    });

    const from = this.getURL(
      path.relative(this.publicPath, path.dirname(dist)).split(path.sep).join('/')
    );
  ...
  _readDoc(filePath) {
    ...
    const address = this.getURL(targetRelative);

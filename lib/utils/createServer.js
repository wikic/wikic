...
module.exports = async ({ getCwd = () => process.cwd(), getBaseurl = () => '', port = 4000 }) => {
  ...
  if (cachedPort !== port) {
    logger.warn(`Port ${port} is not available. Use ${cachedPort} instead.`);
  }
  server = http.createServer(async (req, res) => {
    ...
    const stats = await fse.stat(pathname);
    ...
    try {
      const data = await fse.readFile(pathname);
